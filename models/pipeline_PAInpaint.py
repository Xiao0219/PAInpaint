...
class PAInpaintPipeline(
    ...
    def __call__(
        ...
        image: PipelineImageInput = None, # masked_image
        mask_image: PipelineImageInput = None, # 使用了kwargs，其实是generate传入的mask
        ...
        source_image: PipelineImageInput = None, # ref_image
        referencenet = None,
        ...
        **kwargs,
        ...
        # latents_outputs = [masked_image_latents按时间步加噪，noise，masked_image_latents]
        # source_image_latents = ref_image_latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print("******************************")
                # print("do_classifier_free_guidance之后的Dimensions:") 
                # print("latent_model_input:", latent_model_input.shape)       # torch.Size([2, 4, 64, 64])
                # print("mask:", mask.shape)                                   # torch.Size([2, 1, 64, 64])
                # print("masked_image_latents:", masked_image_latents.shape)   # torch.Size([2, 4, 64, 64])
                # print("depth_feature:", depth_feature.shape)                 # torch.Size([1, 4, 64, 64])
                # print("******************************")
                ...
                if i >= 0:
                    ...
                    referencenet(
                        ...
                        t,
                        #torch.zeros_like(t),
                        encoder_hidden_states=clip_image_embed, #torch.Size([2, 257, 768])
                        return_dict=False,
                    )
                    if reference_control_reader is not None and reference_control_writer is not None:
                ...
                # print("##############################")
                # print("latent_model_input:", latent_model_input.shape) #torch.Size([2, 13, 64, 64])
                # print("t:", t)
                # print("prompt_embeds:", prompt_embeds.shape) #torch.Size([2, 257, 768])
                # print("##############################")
                noise_pred = self.unet(
                    latent_model_input, #torch.Size([2, 13, 64, 64])
                    t,
                    encoder_hidden_states=prompt_embeds, #torch.Size([2, 257, 768])
